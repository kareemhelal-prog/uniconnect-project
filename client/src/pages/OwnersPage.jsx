...
export default function OwnersPage() {
  ...
  const fetchOwners = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        const mapped = res.data.users.map((item) => ({
          id:        item.id,
  ...
  if (loading)
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "60vh", flexDirection: "column", gap: "1rem" }}>
        <div style={{ width: 48, height: 48, border: "3px solid rgba(0,229,255,0.2)", borderTop: "3px solid #00e5ff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        <p style={{ color: "var(--text-muted)", fontFamily: "'Rajdhani',sans-serif", letterSpacing: 2 }}>
          LOADING OWNERS...
        </p>
    ...
    <main className="owners-page">
      ...
      <div className="owners-card">
        <div className="card-header">
          ...
          <button className="card-collapse-btn" onClick={fetchOwners} title="Refresh">
            <IconRefresh />
          </button>
        </div>
        ...
        <div className="entries-info">
          Showing {Math.min(1, sorted.length)} to {Math.min(entries, sorted.length)} of {sorted.length} entries
          ...
          <table className="owners-table">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort("firstName")}>First Name<SortIcon field="firstName" /></th>
                <th onClick={() => handleSort("lastName")}>Last Name<SortIcon field="lastName" /></th>
                ...
                <tr key={owner.id}>
                  ...
                  <td>
                    <div className="action-btns">
                      <button className="action-btn accept-btn" onClick={() => handleAccept(owner.id)} title="Accept">
                        <IconCheck /> Accept
                      </button>
                      <button className="action-btn reject-btn" onClick={() => handleReject(owner.id)} title="No Accept">
                        <IconX /> No Accept
                      </button>
                      <button className="action-btn delete-btn" onClick={() => handleDelete(owner.id)} title="Delete">
                        <IconTrash /> Delete
                      </button>
                    </div>
                  </td>
