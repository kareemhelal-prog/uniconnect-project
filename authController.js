...
exports.register = async (req, res) => {
  const { name, email, password, username, role, doctorYear, specialization } = req.body;
  ...
  if (role === "student" && (!doctorYear || !specialization)) {
    return res.status(400).json({
  ...
  try {
    ...
    if (role === "student") {
      await connection.query(
        ...
        [userId, "General Faculty", specialization, doctorYear]
      );
    } else if (role === "doctor") {
      await connection.query(
